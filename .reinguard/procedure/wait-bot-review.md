...
5. For `waiting_bot_rate_limited`, compute **`cooldown_sec`** from `rate_limit_remaining_seconds` when present; else parse it from the **selected status comment** (same `status_comment_at` / `status_comment_source` as in Act step 1). **Sleep `cooldown_sec + 30` seconds** before posting `@coderabbitai review` — **30** matches the local gate default **`RATE_LIMIT_RETRY_BUFFER_SEC`** in `.reinguard/scripts/check-local-review.sh` (same formula as local `--retry-on-rate-limit`). Then follow the one-retry recovery path in `review--bot-operations.md` instead of the generic 30-second poll cadence during the cool-down window.
