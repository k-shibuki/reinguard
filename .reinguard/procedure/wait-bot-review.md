...
5. For `waiting_bot_run`, poll every **30 seconds** for up to **20 minutes**. Stop immediately if the required bot becomes terminal, actionable review work appears, or the FSM should hand off to another procedure. If you (or an agent) just posted `@coderabbitai review` / another configured **`review_triggers`** pattern, continue polling until **`bot_review_trigger_awaiting_ack`** clears in `bot_review_diagnostics` / `bot_reviewer_status` (fresh bot acknowledgement on the timeline), not merely until a fixed delay elapses.
