...
1. **Sense** — `rgd context build --compact` by default (same cwd / `--config-dir` as the workflow’s initial context build from repo root). Fall back to full `rgd context build` only when you genuinely need the full nested observation payload.
