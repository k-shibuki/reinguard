...
- Prefer repo-local writable state under `/.tmp/` for local verification commands (for example via `.reinguard/scripts/with-repo-local-state.sh`) instead of depending on user-home cache paths.
