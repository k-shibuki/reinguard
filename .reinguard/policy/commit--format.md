...
- Machine validation: `commit-msg` stage in `.pre-commit-config.yaml` (runs `.reinguard/scripts/check-commit-msg.sh`). This script path is the SSOT for validation, and the script is not invoked via `make`.
