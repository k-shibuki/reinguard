...
//
// CodeRabbit operational wrapper comments are treated as non-findings so that auto-generated
// acknowledgements and status traffic do not keep non-thread finding gates latched after a
// disposition was posted on the PR conversation.
...
var (
	...
	reCRAutoGeneratedComment     = regexp.MustCompile(`(?i)<!--\s*this\s+is\s+an\s+auto-generated\s+comment:\s*[^>]*coderabbit(?:\.ai)?\s*-->`)
...
func CoderabbitIssueCommentMaxTier(body string) int {
	...
	t := 0
	if reCRAutoGeneratedReply.MatchString(body) || reCRAutoGeneratedComment.MatchString(body) {
		t = max(t, 6)
	}
