...
| `--serial` | — | Observation commands only. Run providers sequentially (default: parallel) |
| `--fail-on-non-resolved` | — | Supported evaluation commands only. Exit code `2` when the outcome is `ambiguous`, `degraded`, or `unsupported` |
| `--trace-rules` | — | `state eval`, `route select`, and `context build` only. Include a rule-level evaluation trace (`rule_trace[]`) in the JSON output. Default off; default output is unchanged. |
...
Raw `rgd observe` commands do not support `--fail-on-non-resolved`; provider
degradation is reported in the observation document and interpreted only by
higher-level evaluation commands.
...
| `github` | `bot_reviewers` | array | Optional. Each element: `id` (string, required, `^[a-z0-9_]+$`, unique), `login` (string, required), `required` (boolean, required — whether this bot participates in aggregate diagnostics), `enrich` (optional string array of built-in enrichment names), `review_triggers` (optional string array — Go `regexp` patterns matched against PR **conversation** comments **not** authored by this bot’s `login`). When a match is newer than the bot’s latest status acknowledgement, observation sets `review_trigger_awaiting_ack` and keeps the bot **pending** until the bot posts a fresher acknowledgement (issue comment and/or `latestReviews.submittedAt`). Drives `signals.github.reviews.bot_reviewer_status` and `bot_review_diagnostics`. The same configured set is used by top-level `rgd observe`, by `rgd observe github reviews`, and by `rgd context build` including compact output; provider filtering must not drop these options. Unknown `enrich` names fail `rgd config validate` / provider build; invalid `review_triggers` patterns fail provider build at compile time. Built-in enrichments: `coderabbit` (rate-limit seconds, CodeRabbit Review Status markers, review-body and conversation-comment markers per `docs/cli.md`, `StatusClassifier`). |
...
- **`bot_review_trigger_awaiting_ack`:** true when any **required** bot has `review_trigger_awaiting_ack` in `bot_reviewer_status` (manual or agent re-review trigger visible on the PR timeline without a newer bot acknowledgement). The built-in **`merge-readiness`** guard fails closed when this is true or missing.
...
| `--check-json` | no* | Inline JSON for one check object or an array of check objects. This is a single option, not repeatable; use an array when supplying multiple checks. |
| `--checks-file` | no* | JSON array of check objects with fields `id`, `status`, required `summary`, and optional `evidence`; check `status` may be `pass`, `fail`, or `skipped`. Use `-` to read the JSON array from stdin. |
...
\* At least one check entry is required from `--check`, `--check-json`, or `--checks-file` (or any combination). These inputs may be combined; entries are merged.

Preferred patterns:

- Simple checks: repeat `--check`
- One-off evidence-rich checks: use `--check-json` with one object, or one array for multiple checks
- Many checks generated by another command: pipe a JSON array to `--checks-file -`
...
| 6 | `github.reviews.bot_review_diagnostics.bot_review_trigger_awaiting_ack` | `== false` | bot review trigger awaiting acknowledgement |
| 7 | `github.reviews.bot_review_diagnostics.bot_review_terminal` | `== true` | required bot review not terminal |
| 8 | `github.reviews.bot_review_diagnostics.bot_review_failed` | `== false` | required bot review failed |
| 9 | `github.reviews.bot_review_diagnostics.bot_review_stale` | `== false` | required bot review is stale or missing review commit SHA |
| 10 | `github.reviews.review_decisions_changes_requested` | `== 0` | changes requested: N |
| 11 | `github.reviews.pagination_incomplete` | `== false` | review thread pagination incomplete |
| 12 | `github.reviews.review_decisions_truncated` | `== false` | review decisions truncated |
| 13 | `github.pull_requests.merge_state_status` | `== "clean"` (case-insensitive) | merge state status is "X", want clean |
| 14 | `github.ci.ci_status` | `== "success"` (case-insensitive) | ci status is "X", want success |
...
| `--observation-file FILE` | Optional. Observation JSON (same shape as `rgd observe` stdout). When set, the file’s `signals` object is flattened first, and entries are kept only if `when` matches that flat signal map (not state-resolved or gate-enriched; use `context build` for `state.*` or `gates.*` paths in `when`). |
