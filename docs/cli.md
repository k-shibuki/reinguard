...
- **Body substring flags (case-insensitive):** `contains_rate_limit`, `contains_review_paused`, `contains_review_failed`. These flags are computed from the **latest PR issue comment** for that bot login (`latestCommentForLogin` in the observation builder): only that comment body is scanned. If the bot posts a new comment after a rate-limit message (e.g. an acknowledgment), `contains_rate_limit` returns **false** on the next observation and the FSM can leave `waiting_bot_rate_limited` when other signals allow.
