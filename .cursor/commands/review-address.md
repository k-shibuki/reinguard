...
For **Acknowledged** specifically — **before** posting the thread reply: read `.reinguard/policy/review--consensus-protocol.md` § **Acknowledged — in-PR resolution vs follow-up Issue**. Open a **new** GitHub Issue with `Tracked in #…` **only** when deferred work is a **large, separately scoped** deliverable; otherwise use **Fixed** / **By design** / **False positive**, or **Acknowledged** with a written rationale and **no** new Issue when the protocol allows. Never use `Tracked in #N` where `N` equals the PR’s `Closes` issue.
